...
downloadHtmlwidgetUI <- function(id, text = "Download", formats = NULL, class = NULL, display = c("buttons", "dropdown"),
  ...
  if (display == "dropdown") {
    div(modal(id = paste0("md-", ns("DownloadWidgetlink")), title = modalTitle, modal_content),
    ...
    shiny::div(shiny::tagList(shiny::singleton(shiny::tags$body(shiny::tags$script(src = "downloadInfo/downloadGen.js")))),
               modal(id = "md-button_html-DownloadWidgetlink", title = modalTitle, modal_content),
...
#' @export
downloadHtmlwidget <- function(input, output, session, widget = NULL, formats, name = "widget", modalFunction = NULL, ...) {
