...
downloadHtmlwidgetUI <- function(id, text = "Download", formats = NULL, class = NULL, display = c("buttons", "dropdown"),
  ...
  if (display == "dropdown") {
    div(modal(id = "md-dropdown_html-DownloadWidgetlink", title = modalTitle, modal_body),
    ...
    shiny::div(shiny::tagList(shiny::singleton(shiny::tags$body(shiny::tags$script(src = "downloadInfo/downloadGen.js")))),
               modal(id = "md-button_html-DownloadWidgetlink", title = modalTitle, modal_body),
...
#' @export
downloadHtmlwidget <- function(input, output, session, widget = NULL, formats, name = "widget", modalFunction = NULL, modalFunctionArgs = list()) {
