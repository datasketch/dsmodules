#'@export
downloadTableUI <- function(id, text = "Download", formats = NULL, class = NULL, label = "Download", display = c("buttons", "dropdown"), dropdownWidth = 150) {
  ...
  if (display == "dropdown") {
    ch <- ns(paste0("DownloadTbl", tbl_formats))
    names(ch) <- paste0(text, " ", toupper(tbl_formats))
    ...
    div(shiny::tagList(shiny::singleton(shiny::tags$body(shiny::tags$script(src = "downloadInfo/downloadGen.js")))),
        lapply(tbl_formats, function(z) {
          shiny::tagList(shiny::div(style = "text-align: center;",
                                    ...
                                    shiny::downloadButton(ns(paste0("DownloadTbl", z)), paste0(text, " ", toupper(z)), class = class, style = "width: 200px; display: inline-block;"),
