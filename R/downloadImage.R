#' @export
downloadImageUI <- function(id, text = "Download", formats = NULL, class = NULL, label = "Download", display = c("buttons", "dropdown"), dropdownWidth = 150) {
  ...
  if (display == "dropdown") {
    ch <- ns(paste0("DownloadImg", img_formats))
    names(ch) <- paste0(text, " ", toupper(img_formats))
